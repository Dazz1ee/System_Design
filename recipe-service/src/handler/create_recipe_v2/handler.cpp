...
void Validate(const schemas::CreateRecipeRequestV2DTO& request) {
...
std::string Handler::HandleRequestThrow(
  ...
  const auto request_dto = request_json.As<schemas::CreateRecipeRequestV2DTO>();
  Validate(request_dto);

  auto user_id = context.GetData<int64_t>(auth::jwt::JwtUtils::kClaim);

  auto response_dto = recipe_service_.CreateRecipeV2(request_dto, user_id);
