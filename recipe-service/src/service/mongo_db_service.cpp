...
MongoDbService::MongoDbService(
    ...
    : ComponentBase(config, context),
      mongo_(context.FindComponent<userver::components::Mongo>("mongo-db")
                 .GetPool()),
      mongo_write_(
          context.FindComponent<userver::components::Mongo>("mongo-write-db")
              .GetPool()) {}

entity::MongoRecipe MongoDbService::CreateRecipeV2(
    const entity::MongoRecipe& recipe) const {
  auto ingredients_col = mongo_write_->GetCollection("ingredients");
  ...
  auto cursor = ingredients_col.Find(
      MakeDoc("_id", MakeDoc("$in", ids_builder.ExtractValue())));
  ...
  userver::formats::bson::ValueBuilder ing_builder(
      userver::formats::common::Type::kArray);
  for (const auto& ingredient : recipe.ingredients) {
    ...
    ing_builder.PushBack(MakeDoc(
        ...
        "name", name_map[ingredient.ingredient_id], "amount",
        ingredient.amount.value_or(0.0), "unit", ingredient.unit.value_or("")));
  }
  ...
  auto outbox_doc =
      MakeDoc("_id", userver::formats::bson::Oid{}, "event_type",
              "RecipeCreated", "aggregate_id", oid.ToString(), "status", "NEW",
              "created_at", created_at, "attempts", 0, "payload", doc);

  auto transaction = mongo_write_->BeginTransaction();
  try {
    auto recipes_col = transaction.GetCollection("recipes");
    auto outbox_col = transaction.GetCollection("outbox");
    auto result = recipes_col.InsertOne(doc);
    outbox_col.InsertOne(outbox_doc);
    transaction.Commit();
...
entity::MongoRecipe MongoDbService::CreateRecipe(
  ...
  auto doc =
      MakeDoc("_id", userver::formats::bson::Oid{recipe.id}, "title", recipe.title, "description",
              recipe.description ? *recipe.description : "", "servings",
              recipe.servings.value_or(1), "cook_time_minutes",
              recipe.cook_time_minutes.value_or(0), "author_id",
              std::to_string(recipe.author_id), "created_at", created_at,
...
std::vector<entity::MongoRecipe> MongoDbService::GetRecipes(
    std::string& last_id, int limit) const {
  auto recipes_col = mongo_->GetCollection("recipes");
  ...
  if (!last_id.empty()) {
    query_builder["_id"] = MakeDoc("$gt", userver::formats::bson::Oid{last_id});
  }
  ...
  for (const auto& doc : cursor) {
    ...
    entity.created_at =
        doc["created_at"].As<std::chrono::system_clock::time_point>();

    entity.description = doc["description"].As<std::optional<std::string>>();
    entity.servings = doc["servings"].As<std::optional<int>>();
    entity.cook_time_minutes =
        doc["cook_time_minutes"].As<std::optional<int>>();

    entity.steps = doc["steps"].As<std::vector<entity::RecipeStep>>({});
    entity.ingredients =
        doc["ingredients"].As<std::vector<entity::MongoRecipeIngredient>>({});
...
std::vector<entity::MongoRecipeIngredient> MongoDbService::GetRecipeIngredients(
    const std::string& recipe_id) const {
  auto collection = mongo_->GetCollection("recipes");

  auto filter = MakeDoc("_id", userver::formats::bson::Oid{recipe_id});
...
userver::storages::mongo::Cursor MongoDbService::GetOutbox() const {
  auto outbox_col = mongo_write_->GetCollection("outbox");

  return outbox_col.Find(MakeDoc("status", "NEW"));
}

void MongoDbService::SetOutboxSuccessStatus(
    const userver::formats::bson::Oid& oid) const{
  auto outbox_col = mongo_write_->GetCollection("outbox");

  outbox_col.UpdateOne(
      MakeDoc("_id", oid),
      MakeDoc("$set", MakeDoc("status", "DONE", "published_at",
